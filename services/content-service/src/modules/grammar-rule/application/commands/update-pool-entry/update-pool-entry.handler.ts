...
import { UpdatePoolEntryCommand } from './update-pool-entry.command.js';
...
@CommandHandler(UpdatePoolEntryCommand)
export class UpdatePoolEntryHandler implements ICommandHandler<
...
> {
  ...
  async execute(command: UpdatePoolEntryCommand): Promise<Result<void, GrammarRuleDomainError>> {
    ...
    const weightResult = entry.updateWeight(command.weight);
    if (weightResult.isFail) {
      return Result.fail(weightResult.error);
    }

    await this.poolRepo.save(entry);
