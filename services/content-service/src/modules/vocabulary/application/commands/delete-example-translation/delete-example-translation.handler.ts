...
import { DeleteExampleTranslationCommand } from './delete-example-translation.command.js';
...
@CommandHandler(DeleteExampleTranslationCommand)
export class DeleteExampleTranslationHandler implements ICommandHandler<
...
> {
  ...
  async execute(
    command: DeleteExampleTranslationCommand,
    ...
    const translation = await this.exTranslationRepo.findByExampleAndLanguage(
      command.exampleId,
      command.translationLanguage,
    );
    if (!translation) {
      return Result.fail(VocabularyDomainError.EXAMPLE_TRANSLATION_NOT_FOUND);
    }

    await this.exTranslationRepo.delete(translation.id);
