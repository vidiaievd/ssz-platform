...
import { DeleteUsageExampleCommand } from './delete-usage-example.command.js';
...
@CommandHandler(DeleteUsageExampleCommand)
export class DeleteUsageExampleHandler implements ICommandHandler<
...
> {
  ...
  async execute(command: DeleteUsageExampleCommand): Promise<Result<void, VocabularyDomainError>> {
    ...
    // Hard delete — cascades to example translations via DB ON DELETE CASCADE.
    await this.exampleRepo.delete(command.exampleId);
