...
export class RabbitmqEventPublisher implements IEventPublisher, OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    ...
    if (!url) {
      this.logger.warn(
        'RABBITMQ_URL is not configured — event publishing is disabled. Set RABBITMQ_URL to enable.',
      );
      return;
    }
    ...
    this.channelWrapper = this.connection.createChannel({
      setup: async (channel: ConfirmChannel) => {
        // Declare exchange — idempotent, safe to call on every reconnect
  ...
  async publish<T>(
    ...
    if (!this.channelWrapper) {
      this.logger.warn(
        `Cannot publish "${eventType}" — RabbitMQ is not connected (RABBITMQ_URL not set)`,
      );
      return;
    }
    ...
    try {
      // Routing key = eventType (e.g. "content.container.published")
