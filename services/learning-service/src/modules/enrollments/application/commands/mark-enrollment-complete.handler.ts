...
import { MarkEnrollmentCompleteCommand } from './mark-enrollment-complete.command.js';

@CommandHandler(MarkEnrollmentCompleteCommand)
export class MarkEnrollmentCompleteHandler
  implements ICommandHandler<MarkEnrollmentCompleteCommand, Result<EnrollmentDto, EnrollmentApplicationError>>
{
  private readonly logger = new Logger(MarkEnrollmentCompleteHandler.name);
  ...
  async execute(cmd: MarkEnrollmentCompleteCommand): Promise<Result<EnrollmentDto, EnrollmentApplicationError>> {
    ...
    if (enrollment.userId !== cmd.requestingUserId) {
      return Result.fail(new EnrollmentForbiddenError('only the enrolled user can mark completion'));
    }

    const result = enrollment.complete(this.clock.now());
    if (result.isFail) {
    ...
    this.logger.log(`Enrollment ${cmd.enrollmentId} marked complete`);
