...
import type { CheckMode, ExerciseDefinition } from '../../shared/application/ports/content-client.port.js';
...
export class ExerciseDefinitionCache {
  ...
  // mode is part of the key — a PRACTICE entry has expectedAnswers, a GRADED one doesn't,
  // so the two must never be served interchangeably.
  private cacheKey(exerciseId: string, language: string, mode: CheckMode): string {
    return `exercise-def:${exerciseId}:${language}:${mode}`;
  }
  ...
  async get(
    ...
    const raw = await client.get(this.cacheKey(exerciseId, language, mode));
  ...
  async set(
    ...
    await client.set(
      this.cacheKey(exerciseId, language, mode),
