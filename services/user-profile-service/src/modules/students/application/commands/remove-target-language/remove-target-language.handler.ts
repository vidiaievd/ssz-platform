...
import { RemoveTargetLanguageCommand } from './remove-target-language.command.js';

@CommandHandler(RemoveTargetLanguageCommand)
export class RemoveTargetLanguageHandler implements ICommandHandler<RemoveTargetLanguageCommand> {
  constructor(
  ...
  async execute(command: RemoveTargetLanguageCommand): Promise<void> {
    ...
    studentProfile.removeTargetLanguage(command.languageCode);
