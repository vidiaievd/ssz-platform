...
export class RabbitmqConsumerService implements OnModuleInit, OnModuleDestroy {
  ...
  private async handleMessage(channel: ConfirmChannel, msg: ConsumeMessage | null): Promise<void> {
    ...
    let envelope: { eventId?: string; eventType?: string; occurredAt?: string; source?: string; correlationId?: string; payload?: unknown };
    ...
    if (alreadyProcessed) {
      this.logger.debug(`Duplicate event "${eventType}" [${eventId}] — acking (already processed)`);
      channel.ack(msg);
      return;
    }
    ...
    try {
      ...
      // Requeue once; dead-letter on second failure
